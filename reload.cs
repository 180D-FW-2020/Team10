...
{
    //Reloading the bow activity
    Reload reloadX;
    //public Transform firePoint;
    //public GameObject bulletPrefab;
    //public float offset;
        //attempr at adding ammo
    ...
    //remember to delete matColor because is not even being used
    ...
    void Update()
    {
        //here I will call the update whatever is the ammo object to the
        //current ammo
    ...
    void ReadCallback(IAsyncResult ar)
    {
        ...
        if (read > 0)
        {
            state.Code.Append(Encoding.ASCII.GetString(state.buffer, 0, read));
            handler.BeginReceive(state.buffer, 0, StateObject.BufferSize, 0, new AsyncCallback(ReadCallback), state);
        }
        else
        {
            ...
            { 
                string content = state.Code.ToString();
                print($"Read {content.Length} bytes from socket.\n Data : {content}");
                //Reload();
                reloadX.reload = int.Parse(content)
                //SetColors(content);
            }
            handler.Close();
        }
